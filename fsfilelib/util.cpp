...
inline void _CopyCallback( FSOP *pfsop )
{
	...
	if( pfsop->lpCallback )
	{
		Return = pfsop->lpCallback(
			pfsop->TotalFileSize,
			pfsop->TotalBytesTransferred,
			pfsop->StreamSize,
			pfsop->StreamBytesTransferred,
			pfsop->dwStreamNumber,
			0, // CALLBACK_CHUNK_FINISHED, CALLBACK_STREAM_SWITCH
			pfsop->hSourceFile,
			pfsop->hDestinationFile,
			pfsop->lpData
			);
	...
	)
{
	...
	__try
	{
		...
		while( TRUE )
		{
			...
			if( dwStreamId != pStreamID->dwStreamId )
			...
			//
			// Notifies to caller that stream switched.
			//
			_CopyCallback( pfsop );
			if( pfsop->Abort )
			{
	...
	)
{
	...
	pfsop->lpCallback = lpProgressRoutine;
	pfsop->lpData = lpData;

	FILE_STREAM_INFORMATION *StreamInformation = NULL;
	INT cAltStream;
	if( GetAlternateStreamInformation(hSrcFile,&cAltStream,&StreamInformation) == STATUS_SUCCESS )
	{
		if( cAltStream > 0 )
		{
			...
			for(int i = 0;;i++)
			{
				pfsop->TotalFileSize.QuadPart += ps->StreamSize.QuadPart;
	...
	{
		GetFileSizeEx(hSrcFile,&pfsop->TotalFileSize);
	}
