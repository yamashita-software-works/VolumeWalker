...
- Volume File ExplorerのSimple Binary Dumpの更新。
  - ファイルを開く際、ファイルに代替ストリームが含まれる場合、ストリームを選択して開ける様にした。
  - ディレクトリに代替ストリームが割り当てられている場合にもストリームを選択して開ける様にした。

- fsfilelib.dllを再構成。エクスポーズされた関数名を変更・整理中。

- volumeinfo.exeのメニューを更新。

- volumeinfo.exeのビューに"Relation View"を追加。

- volumewalker.exe コマンドライン変更。
  - /nx オプションの追加。
  - /nオプション指定時に/weを打ち消し

- libmisc,libntwdkのソースコード整理。
...
- Volume File Explorerからファイルのドラッグを可能にした（試験的な機能の有効化）。

### 0.1.52.0

- Dos　Drives ウィンドウのコンテキストメニューに "Action" 項目（サブメニュー）を追加。   
サブメニュー下に、次のコマンドを追加しました。Volumesウィンドウに存在するものと同じです。

    - \[Edit Volume Label\] ドライブのボリュームラベルの編集
    - \[Edit Volume Object Id\] ドライブのボリュームObjectIDの編集
    - \[Mount to Folder\] フォルダへドライブをマウント
    - \[Lookup Stream Name by LCN\] LCNからストリーム名をドライブ上で調べる

- Volume File Explore ウィンドウから、簡易バイナリビューアーダイアログを開くコマンドを追加。   
リスト上の項目を選択してコンテキストメニュー開き、\[Simple Binary Dump\]を選択します。

- Win32のリストビューコントロールを使用している場所で、フォーカスを持つ選択項目行の左端がリストビューのウィンドウ外に出ている時、メニューキー（またはShift+F10）を押してコンテキストメニューを表示すると正しい位置に表示されない不具合を修正。
フォーカスを持つ選択項目行が完全にウィンドウ外にある場合、コンテキストメニューは表示しない様にした。

- Volume Drive Relation View ウィンドウのWin32 ツリーコントロールで、選択された項目の矩形がツリービューのウィンドウの外に出ている、または矩形の一部のがウィンドウの外に出ている時、メニューキー（またはShift+F10）を押してコンテキストメニューを表示すると正しい位置に表示されない不具合を修正。
選択された項目の矩形が完全にウィンドウ外にある場合、コンテキストメニューは表示しない様にした。

- Volume File Explorer ウィンドウで、ファイルやディレクトリのサイズが0の場合の\[Size\]列と\[Allocation Size\]列の表示を変更。
...
- Physical Drivesウィンドウを表示中は各ドライブをオープンしたままだった仕様を、初期表示時や更新時の情報収集後はクローズする様に変更した。

- Volume Files Browserウィンドウで、NTFS以外のファイルシステムで[Cluster Information]を実行すると「パラメータが間違っています」とエラーが表示される不具合を修正。

- Volume Files Browserウィンドウで、クラスタ、物理ドライブ、物理ドライブオフセットの表示を改善。

- Simple Hex Dumpウィンドウのヘッダ表示を変更。

- その他ソースコード、ライブラリ整理・更新。

### 0.1.44.0

- Volume Files Browserウィンドウのソース構成、ビルド環境を更新。   
開発実験版と同じ基盤の構成に移行しました。

- Volume Files BrowserウィンドウのCluster Informationで、ミラーボリューム上のファイルの情報を正しく表示できない不具合を修正。

- libmiscライブラリの更新。

### 0.1.43.1

- volumeinfo.exeで項目選択ペインから物理ドライブを選択した時、物理ドライブ情報ページが更新されない不具合を修正。   
本修正に関連したモジュールのみ0.1.43.1に更新されています。
