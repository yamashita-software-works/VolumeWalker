...
PWSTR WINAPI GetGPTPartitionTypeString( GUID& Guid, LPWSTR pszBuf, int cchBuf );
...
PWSTR GetDependentDiskFlagString(DWORD Flag,PWSTR psz,DWORD cch);
...
EXTERN_C LONG WINAPI StorageGetDeviceNumber(HANDLE hFile, PSTORAGE_DEVICE_NUMBER DeviceNumber);
EXTERN_C LONG WINAPI StorageGetDeviceIdDescriptor(HANDLE hFile,PSTORAGE_DEVICE_DESCRIPTOR *pDeviceDescriptor);
EXTERN_C DWORD WINAPI StorageDetectSectorSize(HANDLE hFile, PSTORAGE_ACCESS_ALIGNMENT_DESCRIPTOR pAlignmentDescriptor);
LONG StorageGetMediaTypes(HANDLE hFile, PGET_MEDIA_TYPES *pMediaTypes);
