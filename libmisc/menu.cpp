...
static BOOL _QueryAreEnabledMenuItems(HMENU hMenu,PFNQUERYCOMMANDSTATE pfnUpdateUICallback,LPARAM lParam)
{
	...
	for(i = 0; i < cMenuItems; i++)
	{
		...
		else
		{
			...
			if( state & MF_CHECKED )
				cmdUIState |= UPDUI_CHECKED;

			cmdUIState = pfnUpdateUICallback(id,cmdUIState,&mcs,lParam);
			if( cmdUIState != -1 )
			{
...
VOID UpdateUI_MenuItem(HMENU hMenu,PFNQUERYCOMMANDSTATE pfnQueryCmdStateCallback,LPARAM lParam)
{
	...
	for(i = 0; i < cMenuItems; i++)
	{
		...
		if( State & MF_CHECKED )
			fMenuItemState |= UPDUI_CHECKED;

		fMenuItemState = pfnQueryCmdStateCallback(id,fMenuItemState,&mcs,lParam);
