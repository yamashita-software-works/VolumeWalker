...
public:
	...
	{
		SetWindowText(hWnd,L"CChangeJournalBase");
		return 0;
	}
...
public:
	...
	CPageWndBase* _CreatePage(int nView)
	{
		...
		switch( nView )
		{
			...
			{
				pNew = GetOrAllocWndObjct<CChangeJournalListPage>(nView);
				ASSERT(pNew != NULL);
				break;
			}
...
HRESULT CreateChangeJournalObject(HINSTANCE hInstance,IViewBaseWindow **pObject)
{
	CChangeJournalBase *pWnd = new CChangeJournalBase;

	CChangeJournalBase::RegisterClass(hInstance);
