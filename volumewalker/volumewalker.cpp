...
struct CApplication : public  IApplication
{
	...
	virtual HRESULT QueryInterface(UINT InterfaceId,void **pv)
	{
		if( pv == NULL )
			return E_INVALIDARG;
		...
		{
			case I_LOADCONFIG:
				*pv = new CLoadViewConfig;
				break;
			case I_SAVECONFIG:
				*pv = new CSaveViewConfig;
				break;
		}
		return S_OK;
	}
...
/*++
   ...
   Consoles
    ...
    Single Instance Console:
      ...
      - VOLUME_CONSOLE_VOLUMEFILESEARCHRESULT
      - VOLUME_CONSOLE_VIRTUALDISK
      - VOLUME_CONSOLE_VDSCONSOLE
      - VOLUME_CONSOLE_SCRATCHPAD
...
HWND FindSameWindowType(UINT ConsoleType,PCWSTR pszName)
{
	HWND hwnd;
	hwnd = GetWindow(g_hWndMDIClient,GW_CHILD);
	if( hwnd )
...
HWND FindSameWindowVolume(UINT ConsoleType,PCWSTR pszVolumeName)
{
...
HWND FindSameWindowFilePath(UINT ConsoleType,PCWSTR pszPath)
{
	HWND hwndMDIChildFrame=NULL;
	HWND hwnd;
	hwnd = GetWindow(g_hWndMDIClient,GW_CHILD);
	if( hwnd )
	{
		const int cchCurPath = 32768;
		WCHAR *szCurPath = new WCHAR[cchCurPath];
		...
		do
		{
			...
			if( pcv->wndId == ConsoleType )
			{
				SendMessage(pd->hWndView,PM_GETWORKINGDIRECTORY,cchCurPath,(LPARAM)szCurPath);

				if( _wcsicmp(pszPath,szCurPath) == 0 )
				{
					hwndMDIChildFrame = hwnd;
					break;
				}
			}
...
BOOL IsValidConsoleId( UINT ConsoleTypeId )
{
	switch( ConsoleTypeId )
	{
		...
		case VOLUME_CONSOLE_VIRTUALDISK:
...
HICON GetConsoleIcon(UINT ConsoleTypeId,PCWSTR pszInitialPath)
{
	...
	else if( ConsoleTypeId == VOLUME_CONSOLE_VOLUMEFILES || ConsoleTypeId == VOLUME_CONSOLE_VOLUMEFILESEARCHRESULT )
	{
	...
	else if( ConsoleTypeId == VOLUME_CONSOLE_SCRATCHPAD )
	{
		SHSTOCKICONINFO shsi = {0};
		shsi.cbSize = sizeof(shsi);
...
VOID SendFileListPath(HWND hwndMDIChild,UINT ConsoleTypeId,MDICHILDWNDDATA *pd,OPEN_MDI_CHILDFRAME_PARAM *pOpenParam)
{
	...
	_SafeMemFree(pg.Context);
	_SafeMemFree(pszPath);
}

VOID SendFileSearchResult(HWND hwndMDIChild,UINT ConsoleTypeId,MDICHILDWNDDATA *pd,OPEN_MDI_CHILDFRAME_PARAM *pOpenParam)
{
	ASSERT( pOpenParam != NULL );
...
HWND OpenMDIChild(HWND hWnd,UINT ConsoleTypeId,LPGUID pwndGuid,OPEN_MDI_CHILDFRAME_PARAM *pOpenParam,BOOL bMaximize,MDICHILDFRAMEINIT *pmdiInit)
{
	...
	if( !IsValidConsoleId(ConsoleTypeId) )
		return NULL;
...
#else
			hwndChildFrame = FindSameWindowVolume(ConsoleTypeId,(pOpenParam != NULL && pOpenParam->Path != NULL) ? pOpenParam->Path : NULL);
#endif
			break;
		}
		case VOLUME_CONSOLE_VOLUMEFILESEARCHRESULT:
	...
	if( hwndChildFrame != NULL )
	{
		...
		else if( VOLUME_CONSOLE_VOLUMEFILESEARCHRESULT == ConsoleTypeId && pOpenParam )
		{
			SendFileSearchResult(hwndChildFrame,ConsoleTypeId,pd,pOpenParam);
		}
#if _ENABLE_FILELIST_SINGLETON
		else if( VOLUME_CONSOLE_VOLUMEFILES == ConsoleTypeId && pOpenParam )
		{
...
HWND InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	if( !args.WithoutOpen )
	{
		...
		else
		{
			...
			//
			HRESULT hr;
			MDICHILDFRAMEINIT_VOLUMEWALKER *pLoadedFrames = NULL;
			DWORD dwFrames;
	
			hr = LoadLayout(hWnd,g_hWndMDIClient,&pLoadedFrames,&dwFrames);

			if( hr == S_OK )
			{
				OPEN_MDI_CHILDFRAME_PARAM open_mdi_child_param = {0};

				HWND hwndMDIChild = NULL;
				for(int i = ((int)dwFrames-1); i >= 0; i--)
				{
					MDICHILDFRAMEINIT_VOLUMEWALKER& frame = pLoadedFrames[i];

					if( IsVolumeNameRequiredConsole(&frame.Guid,(UINT)frame.Guid.Data1) && frame.Path == NULL )
						continue;

					if( frame.Guid.Data1 == VOLUME_CONSOLE_VOLUMEFILES )
					{
					...
					open_mdi_child_param.Path = frame.Path;
					open_mdi_child_param.StartOffset.QuadPart = frame.liStartOffset.QuadPart;

					hwndMDIChild = OpenMDIChild(hWnd,
							(UINT)frame.Guid.Data1, // console type id
							&frame.Guid,
							&open_mdi_child_param,
							frame.hdr.show == SW_MAXIMIZE ? TRUE : FALSE,
							&frame.hdr);

					if( hwndMDIChild )
					{
						ShowWindow(hwndMDIChild,frame.hdr.show);
				...
				FreeLayout(pLoadedFrames,dwFrames);
			}
		}
	}
